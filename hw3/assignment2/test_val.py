...
program_command = "valgrind --tool=helgrind ./hw3_2"
...
for i in range(1, iterations + 1):
    ...
    try:
        subprocess.run(program_command.split() + ["8"], stdin=open(input_file), check=True)
    except subprocess.CalledProcessError as e:
        print(f"Error during iteration {i}: {e}")
        # You can choose to handle errors as needed

print("Program has been run %d times." %iterations)
